...
#
# Environment:
#   ICONLIBUTILS_SRCDIR       project root (default: packaging/../..)
#   ICONLIBUTILS_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
ICONLIBUTILS_SRCDIR=${ICONLIBUTILS_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$ICONLIBUTILS_SRCDIR/.config/iconlibutils" "$HOME/.config/iconlibutils"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${ICONLIBUTILS_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${ICONLIBUTILS_PACKAGING:-}")"
...
cmd_run() {
    ...
    if [[ ${ICONLIBUTILS_FORCE_LOCAL:-} == 1 ]]; then
        exec "$@"
    fi
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $ICONLIBUTILS_SRCDIR/.config/iconlibutils/$k.build-host" >&2
        echo "    $HOME/.config/iconlibutils/$k.build-host" >&2
    done
    return 2
}
